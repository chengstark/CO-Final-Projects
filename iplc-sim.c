...
typedef struct cache_line
{
    ...
    int* valid_bit;// a valid bit
    int* tag;// a tag
    // a method for handling varying levels of associativity
    int* frequent;// a method for selecting which item in the cache is going to be replaced
} cache_line_t;
...
void iplc_sim_init(int index, int blocksize, int assoc)
{
    ...
    for (i = 0; i < (1<<index); i++) {
            cache[i].valid_bit = (int*) malloc(sizeof(int)*cache_assoc);
            cache[i].tag = (int*) malloc(sizeof(int)*cache_assoc);
            cache[i].frequent = (int*) malloc(sizeof(int)*cache_assoc);
            for(j=0;j<cache_assoc;j++)
            {
                cache[i].valid_bit[j]=0;
                cache[i].tag[j]=0;
                cache[i].frequent[j]=j;
            }
    }
...
void iplc_sim_LRU_replace_on_miss(int index, int tag)
{
    ...
    int tmp = cache[index].frequent[cache_assoc-1];
    cache[index].tag[tmp] = tag;
    cache[index].valid_bit[tmp] = 1;
    for(int i=cache_assoc-2;i>=0;i--)
    {
        cache[index].frequent[i+1] = cache[index].frequent[i];
    }
    cache[index].frequent[0] = tmp;
}
...
void iplc_sim_LRU_update_on_hit(int index, int assoc_entry)
{
    ...
    {
        if(cache[index].frequent[i]==assoc_entry)
        {
            location = i;
            break;
        }
    ...
    {
        cache[index].frequent[i] = cache[index].frequent[i-1];
    }
    cache[index].frequent[0] = assoc_entry;
}
...
void iplc_sim_finalize()
{
    ...
    {
        free(cache[i].valid_bit);
        free(cache[i].tag);
        free(cache[i].frequent);
    }
